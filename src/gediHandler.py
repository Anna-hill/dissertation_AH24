"""
Script to access simulated GEDI data
"""
...
class gediData(object):
        ...
        if filename:  # then read a file
            ...
            if self.real == 1:
                ...
                (
                    ...
                    self.nPbins,
                    self.zen,
                ) = self.readSimGEDI(filename, minX, maxX, minY, maxY)
    ...
    def readGEDI(self, filename, minX, maxX, minY, maxY):
        ...
        for b in self.beamList:
            ...
            allLat = (
                np.array(f[b]["geolocation"]["latitude_bin0"])
                + np.array(f[b]["geolocation"]["latitude_lastbin"])
            ) / 2.0
            allLon = (
                np.array(f[b]["geolocation"]["longitude_bin0"])
                + np.array(f[b]["geolocation"]["longitude_lastbin"])
            ) / 2.0
            useInd = np.where(
                (allLat >= minY)
                & (allLat <= maxY)
    ...
    def writeSimWaves(self, outRoot, useInd):
        ...
        for i in useInd:
            ...
            cumWave = np.flip(np.cumsum(np.flip(self.wave[i, :], 0)), 0) / np.sum(
                self.wave[i, :]
            )
    ...
    ###########################################
    def readMetricText(self, text_file):
        """Read GEDI_metric text file for plotting elevation
    ...
    def plotSimWaves(self, outRoot, useInd):
        ...
        # loop over waves
        for i in useInd:
            self.true_height1 = self.df.iloc[i, 1]
            self.gedi_height1 = self.df.iloc[i, 5]
            # make z profile
            self.res = (self.Z0[i] - self.ZN[i]) / (self.nBins - 1)
            self.z = np.linspace(self.Z0[i], self.ZN[i], num=self.nBins)
            ...
            # plt.legend()
            plt.axhline(self.true_height1, color="red", label="GEDI ground")
            plt.axhline(self.gedi_height1, color="black", label="True ground")
            plt.legend()
            ...
            if len(binList) > 0:
                if len(binList[0]) > 3:
                    ...
                    for j in range(0, len(binList[0])):
                        if (binList[0][j] == (binList[0][j - 1] + 1)) & (
                            binList[0][j] == (binList[0][j - 2] + 2)
                        ):
                            topBin = binList[0][j]
                            break
...
if __name__ == "__main__":
    ...
    def gediCommands():
        ...
        p.add_argument(
            "--input", dest="inName", type=str, help=("Input GEDI HDF5 filename")
        )
        p.add_argument(
            "--bounds",
            dest="bounds",
        ...
        p.add_argument(
            ...
            action="store_true",
            default=False,
            help=("Write out coordinates insteda of plotting waveforms"),
        )
        p.add_argument(
            "--writeWaves",
            dest="writeWaves",
        ...
        p.add_argument(
            ...
            type=str,
            ## CHANGE to SYSDIR!
            default="/home/s2559258/dissertation/diss_code24/data/teastMetric.metric.txt",
            help=("Write out csv files of the waveforms"),
        )
