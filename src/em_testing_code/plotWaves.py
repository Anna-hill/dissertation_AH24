...
def readCommands():
    ...
    p = argparse.ArgumentParser(
        description=("Specify parameters for program control"),
...
class groundComparison(object):
    ...
    def readMetricFiles(self, rootDir):
        ...
        metric10 = rootDir + "metric98-10/metric{0}-98-10Summary.txt".format(
            args.gridSize
        )
        metric150 = rootDir + "metric98-150/metric{0}-98-150Summary.txt".format(
            args.gridSize
        )
                                ...
                                try:
                                    if index == 0:
                                        plt.axhline(
                                            ...
                                            color="gray",
                                            linestyle="--",
                                            label="Ground Estimates",
                                        )
                                    else:
                                        plt.axhline(
                                            y=ground, color="gray", linestyle="--"
                                        )
                                except:
                                    continue
...
if __name__ == "__main__":
    ...
    file = "data/Bonaly/sim_waves/Sim_320000.0665000.0.h5".format(
        args.site, args.gridSize
    )
    waveforms = "/exports/csce/datastore/geos/groups/3d_env/data/glamis/emitchell/noisedGrids/{0}/grid{1}/addNoise/".format(
        args.site, args.gridSize
    )
    metrics = "/exports/csce/datastore/geos/groups/3d_env/data/glamis/emitchell/noisedGrids/{0}/grid{1}/".format(
        args.site, args.gridSize
    )
