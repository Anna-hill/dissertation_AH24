...
## Anna Notes: editing to work with bonaly test data?
...
# set input metric file locations
rootDir = "data"
inputs = [
    rootDir + "/teastMetric.metric.txt",
...
# inputs with different grid size
# inputs10 = [
...
# input metric files, and set data for each as new var

# does not like wave id? cannot convert to float
# also bonaly.h5 col don't work


bnData = np.loadtxt(inputs[0], dtype=str, comments="#", skiprows=1)
...
bnSlope, _ = readFromFile(slopeInputs[0])
...
for index, datum in enumerate(data):
    # convert str items in txt to float at point of use
    strCover = datum[:, 4]
    floatCover = strCover.astype(float)
    meanCover = np.mean(floatCover)
    sdCover = np.std(floatCover)
    strHeight = datum[:, 5]
    floatHeight = strHeight.astype(float)
    meanHeight = np.mean(floatHeight)
    sdHeight = np.std(floatHeight)
    ...
    print(
        "Canopy cover mean and std. dev.: {0:.2f}, {1:.2f}".format(
            meanCover * 100, sdCover * 100
        )
    )
    print(
        f"Canopy cover > 99%: {0:.2f}%".format(
            (floatCover[floatCover > 0.99]) / datum.shape[0]
        )
        * 100
    )
    print(
        "Canopy cover > 98%: {0:.2f}%".format(
            (len(floatCover[floatCover > 0.98]) / datum.shape[0]) * 100
        )
    )
    print(
        "Canopy height mean and std. dev.: {0:.2f}, {1:.2f}".format(
            meanHeight, sdHeight
        )
    )
...
"""for index, datum in enumerate(data10):
    ...
    print(
        "Canopy cover > 99%: {0:.2f}%".format(
            (len(datum[:, 4][datum[:, 4] > 0.99]) / datum.shape[0]) * 100
        )
    )
    print(
        "Canopy cover > 98%: {0:.2f}%".format(
            (len(datum[:, 4][datum[:, 4] > 0.98]) / datum.shape[0]) * 100
        )
    )
    print(
        "Canopy height mean and std. dev.: {0:.2f}, {1:.2f}".format(
...
for index, subplot in enumerate(subplots):
    subplot.hist(
        ...
        data[index][:, 4],
        bins=np.arange(0.0, 1.0 + 0.1, 0.02),
        density=True,
        edgecolor="black",
        linewidth=0.1,
        label=names[index],
...
for index, subplot in enumerate(subplots):
    subplot.hist(
        data[index][:, 4],
        bins=np.arange(0.6, 1.0 + 0.005, 0.005),
        density=True,
        edgecolor="black",
        linewidth=0.1,
        label=names[index],
...
for index, subplot in enumerate(subplots):
    subplot.hist(
        slopes[index][slopes[index] > -999].flatten(),
